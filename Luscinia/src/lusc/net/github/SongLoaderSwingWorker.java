...
public class SongLoaderSwingWorker extends SwingWorker<SongGroup, Object> {
	...
	public SongGroup doInBackground(){
		...
		if (tabID==1){
			...
			sg=new SongGroup(songs, compScheme, cs.defaults, ac.dbc);
				...
				for (int j=0; j<ac.archIds.length; j++){
					if (q.intValue()==ac.archIds[j]){
						...
						if (song.eleList.size()>0){
							LinkedList<String> list1=ac.dbc.populateContentPane(song.individualID);
			...
			songList=null;
			sg=new SongGroup(songs, cs.defaults, ac.dbc);
