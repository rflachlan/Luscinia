...
public class DisplaySimilarityProportions extends DisplayPane {
	...
	public DisplaySimilarityProportions(SongGroup sg, KMedoids km, int type, int width, int height, Defaults defaults){
		...
		if (km!=null){
			categories=km.overallAssignments;
	...
	public void calculateValues(){
		...
		int[]popIds={0};
		if (type==0){
			scoreLength=sg.scoresEle.length;
			popIds=sg.getPopulationListArray(0);
		}
		else if (type==1){
			scoreLength=sg.scoresEleC.length;
			popIds=sg.getPopulationListArray(1);
		}
		else if (type==2){
			scoreLength=sg.scoresSyll.length;
			popIds=sg.getPopulationListArray(2);
		}
		else if (type==3){
			scoreLength=sg.scoreTrans.length;
			popIds=sg.getPopulationListArray(3);
		}
		else{
			scoreLength=sg.scoresSong.length;
			popIds=sg.getPopulationListArray(4);
		}

		
		for (int i=0; i<scoreLength; i++){
			for (int j=0; j<i; j++){
				if (type==0){score=sg.scoresEle[i][j];}
				else if (type==1){score=sg.scoresEleC[i][j];}
				else if (type==2){score=sg.scoresSyll[i][j];}
				else if (type==3){score=sg.scoreTrans[i][j];}
				else{score=sg.scoresSong[i][j];}
				tot+=score;
				count++;
		...
		for (int i=0; i<scoreLength; i++){
			for (int j=0; j<i; j++){
				...
				if (type==0){
					score=sg.scoresEle[i][j];
					if (sg.lookUpEls[i][0]==sg.lookUpEls[j][0]){
						samesong=true;
					}
				}
				else if (type==1){
					score=sg.scoresEleC[i][j];
					if (sg.lookUpElsC[i][0]==sg.lookUpElsC[j][0]){
						samesong=true;
					}
				}
				else if (type==2){
					score=sg.scoresSyll[i][j];
					if (sg.lookUpSyls[i][0]==sg.lookUpSyls[j][0]){
						samesong=true;
					}
				}
				else if (type==3){
					score=sg.scoreTrans[i][j];
					if (sg.lookUpTrans[i][0]==sg.lookUpTrans[j][0]){
						samesong=true;
					}
				}
				else{
					score=sg.scoresSong[i][j];
				}
	...
	public void calculateLabelledValues(){
		...
		int scoreLength=0;
		if (type==0){scoreLength=sg.scoresEle.length;}
		else if (type==1){scoreLength=sg.scoresEleC.length;}
		else if (type==2){scoreLength=sg.scoresSyll.length;}
		else if (type==3){scoreLength=sg.scoreTrans.length;}
		else{scoreLength=sg.scoresSong.length;}
		...
		for (int i=0; i<scoreLength; i++){
			for (int j=0; j<i; j++){
				...
				if ((place>=0)&&(place<results1.length)){
					if (type==0){
						locDiff=(int)Math.round(numCategories*Math.abs(sg.eleLabels[i]-sg.eleLabels[j]));
						//locDiff=(int)Math.round(Math.abs(sg.lookUpEls[i][1]-sg.lookUpEls[j][1])-1);
						if (locDiff>numCategories){locDiff=numCategories-1;}
					}
					else if(type==1){locDiff=(int)Math.round(numCategories*Math.abs(sg.eleLabels[i]-sg.eleLabels[j]));}
					else if (type==2){locDiff=(int)Math.round(numCategories*Math.abs(sg.syllLabels[i]-sg.syllLabels[j]));}
					else if (type==3){locDiff=(int)Math.round(numCategories*Math.abs(sg.transLabels[i]-sg.transLabels[j]));}
