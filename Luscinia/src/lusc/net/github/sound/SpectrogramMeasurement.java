...
public class SpectrogramMeasurement {
	...
	public void updateSpectrogram(Spectrogram spect) {
		this.spect=spect;
		nout=spect.spect;
	}
	...
	public LinkedList<int[][]> getSignal(int[] pointList1, int[] pointList2, int unx, boolean detectHarmonics){						
		...
		double focal;
		float c=(float)(1/song.spop.dynMax);
		...
		double gapThresh=song.minGap/spect.timeStep;
	...
	public LinkedList<int[][]> eraseSignal(int[] pointList1, int[] pointList2, int currentMinX){						
		...
		su.eleList.removeAll(removeList);
	...
	public void merge(int p, SpectrPane sp, int currentMinX){
		Element ele1=(Element)su.getElement(p);
		Element ele2=(Element)su.getElement(p+1);
		...
		if (diff>0){
			...
			int count=0;
			for (int j=0; j<su.eleList.size(); j++){
				Element ele3=(Element)su.eleList.get(j);
				double p2=ele3.begintime+0.5*(ele3.length);
				if (p2<q){count++;}
				else{j=su.eleList.size();}
			}
			su.eleList.add(count, ele);
		}
	...
	public void calculatePowerSpectrum(LinkedList<int[][]> signal, LinkedList<double[]> results){
		...
		for (int i=0; i<signal.size(); i++){
			...
			double[] powerSpectrum=new double[spect.ny];
			int j=0;
			int a=ele.length;
			...
			for (j=0; j<ele.length; j++){
				...
				for (int k=1; k<ele[j].length; k+=2){
					...
					for (int l=ele[j][k]; l<ele[j][k+1]; l++){
						...
						powerSpectrum[l]+=spect.spectrogram[l][b];
					}
				}
			}
...
void measureFrequencyChange2a(LinkedList<double[][]> freqList, LinkedList<double[][]> freqChangeList){
	...
	public void redodiffs(Song song){
		...
		for (int i=0; i<su.eleList.size(); i++){
		
			Element ele=(Element)su.eleList.get(i);
	...
	private void measureWienerEntropy(LinkedList<int[][]> tList, LinkedList<double[]> results){
		...
		//double maxC=song.spop.dynRange-Math.log(song.maxPossAmp)*logd;
...
//THIS NEEDS CHECKING CAREFULLY!!!! SHOULD USE Spectrogram.spectrogram, not .spect!!!!!!!
//ALSO... what frequency is being used?
	/**
	...
	private void measureAmplitude(LinkedList<double[][]> tList, LinkedList<int[][]> uList, LinkedList<double[]> results){
		...
		for (int i=0; i<elNum; i++){
			...
			for (int j=0; j<eleLength; j++){
				...
				//dat[j]=song.spop.dynRange-nout[v][data2[j][0]];
				
				dat[j]=(Math.log(spect.spectrogram[v][data2[j][0]])-Math.log(spect.maxPossAmp))*logd;
				//SHOULD GIVE AMPLITUDES SCORED BY dB LESS THAN MAX POSS AMPLITUDE				
			}
			results.add(dat);
		}
	}
	...
	void measureReverberation(LinkedList<int[][]> tList, LinkedList<double[]> results){
		...
		int window=(int)Math.round(50/spect.timeStep);
	...
	void measureReverberationRegression(LinkedList<int[][]> tList, LinkedList<double[]> results){
		...
		int elNum=tList.size();
		double timeAmpEquivalence=0.01*spect.timeStep;
		int window=(int)Math.round(100/spect.timeStep);
		if (window<1){window=1;}
	...
	public double[][] calculateTrill5(double[] contours, double max){
		...
		double thresh=0;
		double threshwav=max/spect.timeStep;
		...
		if (m>5){
			...
			for (int i=0; i<n; i++){
				...
				if(loc>=0){
					results[i][0]=amps[loc]*spect.dy;
					//results[i][0]=amps[loc];
				}
				if (results[i][0]>thresh){
					results[i][1]=wavelengths[loc]*spect.timeStep;
	...
	public double[][] calculateTrill2(double[] data, int[][] data2){
		...
		float c=(float)(1/(song.spop.dynMax*song.spop.dynRange));
	...
	public double[][] calculateTrill3(double[] contours, int[][] data, int windowLength){
		...
		for (int i=0; i<n; i++){
			...
			for (int k=0; k<windowLength; k++){
				...
				sumLarge+=Math.max(minna, maxx)-Math.abs(0.5*regb*spect.frameLength/spect.timeStep);
			}

			results[i][0]=sumLarge*spect.dy/(windowLength+0.0);
		}
		return results;
	}
	...
	public double[][] calculateTrill3a(double[] contours, int[][] data, int windowLength){
		...
		for (int i=0; i<n; i++){
			...
			results[i][0]=spect.dy*(sumExp-slopeAdj);
	...
	public double[][] calculateTrill4(double[] contours, int windowLength){
		...
		for (int i=0; i<n; i++){
			...
			results[i][0]=spect.dy*10*(bestDist/wl2);
	...
	public void updateTrillMeasuresX(){
		//System.out.print(su.eleList.size()+" P ");
		LinkedList<Element> eleList=new LinkedList<Element>();
		for (int i=0; i<su.eleList.size(); i++){
			//System.out.println("Q "+i);
			Element ele=(Element)su.eleList.get(i);
	...
	public void checkMinimumLengths(LinkedList<int[][]> signals){
		double x=song.minLength/spect.timeStep;
	...
	public void segment(LinkedList<int[][]> signals, boolean force){
		...
		double r=song.minGap/spect.timeStep;
