...
public class MainPanel extends JPanel implements PropertyChangeListener, ChangeListener, ActionListener, WindowListener{
	...
	public void startDrawing(){
		...
		windowMethod.setSelectedIndex(song.windowMethod-1);
		...
		echoAmt.setValue(new Double(song.echoComp*100));
		echoAmt.setFont(font);
		
		int sco=(int)Math.round(song.echoComp*100);
		if (sco<0){sco=0;}
		if (sco>100){sco=100;}
		...
		noiseAmt.setValue(new Double(song.noiseRemoval));
		noiseAmt.setFont(font);
		
		sco=(int)Math.round(song.noiseRemoval*5);
		if (sco<0){sco=0;}
		if (sco>100){sco=100;}
		...
		dynRange.setValue(new Double(song.dynRange));
		dynRange.setFont(font);
		
		int sgs=(int)Math.round(song.dynRange);
		if (sgs<0){sgs=0;}
		if (sgs>100){sgs=100;}
		...
		dynM.setValue(new Integer((int)Math.round(song.dynMax*100)));
		dynM.setFont(font);
		if (song.setRangeToMax){dynM.setEnabled(false);}
		
		int sgm=(int)Math.round(song.dynMax*100);
		if (sgm<0){sgm=0;}
		if (sgm>100){sgm=100;}
		dynMSL=new JSlider(0, 100, sgm);
		dynMSL.addChangeListener(this);
		dynMSL.setFont(font);
		if (song.setRangeToMax){dynMSL.setEnabled(false);}
		...
		echoRange.setValue(new Double(song.echoRange));
		echoRange.setFont(font);
		
		int stas=(int)Math.round(song.echoRange);
		if (stas<0){stas=0;}
		...
		noiseRange1.setValue(new Double(song.noiseLength1));
		noiseRange1.setFont(font);
		
		stas=(int)Math.round(song.noiseLength1/10);
		if (stas<0){stas=0;}
		if (stas>100){stas=100;}
		...
		noiseRange2.setValue(new Double(song.noiseLength2));
		noiseRange2.setFont(font);
		
		stas=(int)Math.round(song.noiseLength2);
		if (stas<0){stas=0;}
		if (stas>100){stas=100;}
		...
		dynEqual.setValue(new Double(song.dynEqual));
		dynEqual.setFont(font);

		int sdyc=(int)Math.round(song.dynEqual);
		if (sdyc<0){sdyc=0;}
		...
		filterCutOff.setValue(new Double(song.frequencyCutOff));
		filterCutOff.setFont(font);
		
		int sfco=(int)Math.round(song.frequencyCutOff/20.0);
		if (sfco<0){sfco=0;}
		...
		maxFreq.setValue(new Double(song.maxf));
		maxFreq.setFont(font);
		
		int mfs=(int)Math.round(song.maxf/200.0);
		if (mfs<0){mfs=0;}
		if (mfs>100){mfs=100;}
		...
		frameLength.setValue(new Double(song.frameLength));
		frameLength.setFont(font);

		int mfls=(int)Math.round(song.frameLength*5);
		if (mfls<0){mfls=0;}
		...
		timeStep.setValue(new Double(song.timeStep));
		timeStep.setFont(font);
		
		int mts=(int)Math.round(song.timeStep*10);;
		if (mts<0){mts=0;}
		if (mts>100){mts=100;}
		...
		spectOverlap.setValue(new Double(song.overlap*100));
		spectOverlap.setFont(font);
		
		int mto=(int)Math.round(song.overlap*100);
		if (mto<0){mto=0;}
		if (mto>100){mto=100;}
		...
		spectPoints.setValue(new Double(song.frame));
		spectPoints.setFont(font);
		
		int mfps=(int)Math.round(Math.log(song.frame)/Math.log(2));
		if (mfps<0){mfps=0;}
		if (mfps>12){mfps=12;}
		...
		if (song.brushType==1){
			for (int i=0; i<5; i++){
				if (brushSizeInd[i]==song.brushSize){
					brush.setSelectedIndex(i);
				}
			}
		}
		else {
			brush.setSelectedIndex(3+song.brushType);
		}
		brush.setFont(font2);
		...
		fundJumpSuppression.setValue(new Double(song.fundJumpSuppression));
		...
		harmonic.setValue(new Double(song.fundAdjust));
		...
		minGap.setValue(new Double(song.minGap));
		...
		minLength.setValue(new Double(song.minLength));
		...
		upperLoop.setValue(new Double(song.upperLoop));
		...
		lowerLoop.setValue(new Double(song.lowerLoop));
		...
		equipmentField.setText(song.recordEquipment);
		
		recordistField.setFont(font2);
		recordistField.setText(song.recordist);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (LISTEN_TO_DROP_DOWNS){
			if (DELETE_COMMAND.equals(command)) {
				...
				int ind2=song.eleList.size()-ind-1;
				delete.setSelectedIndex(0);
				if (ind>=0){
					song.eleList.remove(ind2);
				}
				if (ind==-1){
					song.eleList=new LinkedList();
				}
				s.paintFound();
				gp.draw();
				updateElementLists();
			...
			if (DELETES_COMMAND.equals(command)) {
				...
				JComboBox cb = (JComboBox)e.getSource();
				int ind=cb.getSelectedIndex()-2;
				int ind2=song.syllList.size()-ind-1;
				
				if (ind>=0){
					song.syllList.remove(ind2);
				}
				if (ind==-1){
			...
			if (PLAY_SPEED_COMMAND.equals(command)) {
				...
				song.playbackDivider=p;
				song.setUpPlayback();
			}
			if (PLAY_COMMAND.equals(command)) {
				if ((player)&&(song.syllList.size()>0)){
					JComboBox cb = (JComboBox)e.getSource();
					int ind=cb.getSelectedIndex()-1;
					int a=0;
					int b=0;
					ind=song.syllList.size()-ind-1;
					int[] syll=(int[])song.syllList.get(ind);
					if (song.ssizeInBits<=16){
						int p1=syll[0]-7;
						if (p1<0){p1=0;}
						int p2=syll[1]+7;
						if (p2>=song.overallLength){
							p2=song.overallLength-1;
						}
						a=(int)(p1*Math.round(song.sampleRate*song.stereo*0.002));
						b=(int)(p2*Math.round(song.sampleRate*song.stereo*0.002));
					}
					if(song.rawData.length<=b){
						b=a;
					}		
		...
		if (PLAY_ALL_COMMAND.equals(command)) {
			if (player){
				song.playSound(0, song.rawData.length);
				drawProgress(0, 1, song.playbackDivider);
			}
		}
		...
		if (PLAY_SCREEN_COMMAND.equals(command)) {
			if (player){
				...
				int a=(int)(2*s.currentMinX*Math.round(s.dx*song.sampleRate*song.stereo*0.001));
				int b=(int)(2*s.currentMaxX*Math.round(s.dx*song.sampleRate*song.stereo*0.001));

				
				song.playSound(a, b);
				drawProgress(s.currentMinX/(song.nx+0.0), s.currentMaxX/(song.nx+0.0), song.playbackDivider);
			}
		}
		...
		if (UNDO_COMMAND.equals(command)) {
			...
			song.eleList=(LinkedList)p.get(0);
			song.syllList=(LinkedList)p.get(1);
		...
		if (WINDOW_COMMAND.equals(command)) {
			JComboBox cb = (JComboBox)e.getSource();
			song.windowMethod= cb.getSelectedIndex()+1;
			resetSpect=true;
		}
		...
		if (ZOOM_TO_ALL_COMMAND.equals(command)){
			double ratio=song.nx/(s.nnx-s.xspace-s.xspace2-0.0);
			ratio=100*ratio;
	...
	void resetBrush(){
		...
		if (c>4){
			...
			if (c==6){
				...
				maxFField.setValue(new Integer(song.maxBrush));
				maxFField.setColumns(10);
				JFormattedTextField minFField = new JFormattedTextField();
				minFField.setValue(new Integer(song.minBrush));
				...
				song.maxBrush=maxb.intValue();
				Integer minb=(Integer)minFField.getValue();
				song.minBrush=minb.intValue();
				song.brushType=3;
			}
		}
		else{
			song.brushType=1;
			song.brushSize=brushSizeInd[c];
		}
	}
	...
	public void updateSyllables(){
		updateSyllableLists();
		System.out.println("NEW SYL LIST: "+song.syllList.size());
		s.paintFound();
		gp.draw();
		cloneLists();
	}
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (started){
			...
			started=false;
			if (source==maxFreq){
				song.maxf = (int)((Number)maxFreq.getValue()).doubleValue();
				if (song.maxf>song.sampleRate/2){song.maxf=(int)song.sampleRate/2;}
				if (song.maxf<1){song.maxf=1;}
				maxFreq.setValue(new Double(song.maxf));
				resetSpect=true;
				int mfs=(int)Math.round(song.maxf/200.0);
				if (mfs<0){mfs=0;}
			...
			if (source==gpMaxF){
				...
				if (guidePanelMaxFrequency>song.sampleRate/2){guidePanelMaxFrequency=(int)song.sampleRate/2;}
			...
			if (source==frameLength){
				song.frameLength = ((Number)frameLength.getValue()).doubleValue();
				if (song.frameLength>1000){song.frameLength=1000;}
				if (song.frameLength<0.1){song.frameLength=0.1;}
				frameLength.setValue(new Double(song.frameLength));
				song.setFFTParameters();
				spectPoints.setValue(new Double(song.frame));
				spectOverlap.setValue(new Double(song.overlap*100));
			...
			if (source==timeStep){
				song.timeStep = ((Number)timeStep.getValue()).doubleValue();
				timeStep.setValue(new Double(song.timeStep));
				song.setFFTParameters();
				spectOverlap.setValue(new Double(song.overlap*100));
				resetSpect=true;
			...
			if (source==spectPoints){
				song.frameLength=((Number)spectPoints.getValue()).doubleValue();
				song.frameLength*=1000/song.sampRate;
				song.setFFTParameters();
				frameLength.setValue(new Double(song.frameLength));
				spectPoints.setValue(new Double(song.frame));
				spectOverlap.setValue(new Double(song.overlap*100));
			...
			if (source==spectOverlap){
				song.overlap=((Number)spectOverlap.getValue()).doubleValue()*0.01;
				if (song.overlap>1){song.overlap=1;}
				song.timeStep=(1-song.overlap)*song.frameLength;
				song.setFFTParameters();
				spectPoints.setValue(new Double(song.frame));
				spectOverlap.setValue(new Double(song.overlap*100));
				timeStep.setValue(new Double(song.timeStep));
			...
			if (source==filterCutOff){
				...
				if (te<0){te=0;}
				if (te>song.maxf){te=song.maxf;}
				song.frequencyCutOff=te;
				resetSpect=true;
				filterCutOff.setValue(new Double(song.frequencyCutOff));
				int sfco=(int)Math.round(song.frequencyCutOff/20.0);;
				if (sfco<0){sfco=0;}
			...
			if (source==dynRange){
				song.dynRange = ((Number)dynRange.getValue()).doubleValue();
				if (song.dynRange<=0){song.dynRange=0.00001;}
				dynRange.setValue(new Double(song.dynRange));
				int sgs=(int)Math.round(song.dynRange);
				if (sgs<0){sgs=0;}
				if (sgs>100){sgs=100;}
			...
			if (source==dynM){
				song.dynMax = (float)((Number)dynM.getValue()).doubleValue();
				song.dynMax*=0.01f;
				if (song.dynMax<=0){song.dynMax=0.00001f;}
				dynM.setValue(new Double(song.dynMax*100));
				int sgs=(int)Math.round(song.dynMax*100);
				if (sgs<0){sgs=0;}
				if (sgs>100){sgs=100;}
				dynMSL.setValue(sgs);
			}
			if (source==echoAmt){
				song.echoComp = ((Number)echoAmt.getValue()).doubleValue();
				song.echoComp*=0.01;
				if (song.echoComp<0){song.echoComp=0;}
			...
			if (source==echoRange){
				song.echoRange = (int)((Number)echoRange.getValue()).doubleValue();
				if (song.echoRange<1){song.echoRange=1;}
				echoRange.setValue(new Double(song.echoRange));
				int stas=(int)Math.round(song.echoRange);
				if (stas<0){stas=0;}
				if (stas>100){stas=100;}
			...
			if (source==noiseAmt){
				song.noiseRemoval = ((Number)noiseAmt.getValue()).floatValue();
				if (song.noiseRemoval<0){song.noiseRemoval=0;}
				noiseAmt.setValue(new Double(song.noiseRemoval));
				int sco=(int)Math.round(song.noiseRemoval*5);
				if (sco<0){sco=0;}
				noiseAmtSL.setValue(sco);
			}
			if (source==noiseRange1){
				song.noiseLength1 = (int)((Number)noiseRange1.getValue()).doubleValue();
				if (song.noiseLength1<1){song.noiseLength1=1;}
				noiseRange1.setValue(new Double(song.noiseLength1));
				int stas=(int)Math.round(song.noiseLength1/10);
				if (stas<0){stas=0;}
				if (stas>100){stas=100;}
				noiseRange1SL.setValue(stas);
			}
			if (source==noiseRange2){
				song.noiseLength2 = (int)((Number)noiseRange2.getValue()).doubleValue();
				if (song.noiseLength2<1){song.noiseLength2=1;}
				noiseRange2.setValue(new Double(song.noiseLength2));
				int stas=(int)Math.round(song.noiseLength2);
				if (stas<0){stas=0;}
				if (stas>100){stas=100;}
				noiseRange2SL.setValue(stas);
			}
			if (source==harmonic){
				song.fundAdjust = ((Number)harmonic.getValue()).doubleValue();
				harmonic.setValue(new Double(song.fundAdjust));
			}
			if (source==fundJumpSuppression){
				song.fundJumpSuppression = ((Number)fundJumpSuppression.getValue()).doubleValue();
				if (song.fundJumpSuppression<0){song.fundJumpSuppression=0;}
				fundJumpSuppression.setValue(new Double(song.fundJumpSuppression));
			}
			if (source==dynEqual){
	...
	void cloneLists(){
		...
		for (int i=0; i<song.syllList.size(); i++){
			int[] syl=(int[])song.syllList.get(i);
			temp2.addLast(syl);
		}
		temp.addLast(temp2);
	...
	public void writeResults(){
		...
		song.location=locationField.getText();
		song.notes=notesField.getText();
		song.recordEquipment=equipmentField.getText();
		song.recordist=recordistField.getText();
		song.sortSyllsEles();
		song.calculateGaps();
	...
	class DrawProgress extends Thread{
		...
		public void run(){
			...
			long currentTime;
			double position;
			double length=song.overallLength;
			while (!stopPlaying){
