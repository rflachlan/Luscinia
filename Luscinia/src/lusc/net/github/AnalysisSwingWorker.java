...
//
//  AnalysisSwingWorker.java
//  Luscinia
//
//  Created by Robert Lachlan on 11/10/11.
...
public class AnalysisSwingWorker extends SwingWorker<String, Object> implements ActionListener {
	...
	public void compressResults(){
		...
		if (anacomp){
			if (sg.scoresEle!=null){
				sg.compressElements();
				System.out.println("COMPRESSING ELEMENTS!");
			}
		}
	...
	public void makeDummyScores(){
		int p=5;
		float[][] mat=new float[sg.scoresSyll.length][p];
		float[][] dmat=new float[sg.scoresSyll.length][];
	...
	public void analyze(){
		...
		if (anael){
			if (anacomp){
				System.out.println("Using eleC"+sg.scoresEleC.length+" "+sg.scoresEle.length);
				tempEle=new float[sg.scoresEleC.length][];
				for (int i=0; i<sg.scoresEleC.length; i++){
					tempEle[i]=new float[sg.scoresEleC[i].length];
					System.arraycopy(sg.scoresEleC[i], 0, tempEle[i], 0, tempEle[i].length);
				}
			}
			else{
				tempEle=new float[sg.scoresEle.length][];
				for (int i=0; i<sg.scoresEle.length; i++){
					tempEle[i]=new float[sg.scoresEle[i].length];
					System.arraycopy(sg.scoresEle[i], 0, tempEle[i], 0, tempEle[i].length);
				}
			}
		...
		if (anasy){
			tempSyll=new float[sg.scoresSyll.length][];
			for (int i=0; i<sg.scoresSyll.length; i++){
				tempSyll[i]=new float[sg.scoresSyll[i].length];
				System.arraycopy(sg.scoresSyll[i], 0, tempSyll[i], 0, tempSyll[i].length);
			}
		...
		if (anast){
			tempSyTr=new float[sg.scoreTrans.length][];
			for (int i=0; i<sg.scoreTrans.length; i++){
				tempSyTr[i]=new float[sg.scoreTrans[i].length];
				System.arraycopy(sg.scoreTrans[i], 0, tempSyTr[i], 0, tempSyTr[i].length);
			}
		...
		if (anaso){
			tempSong=new float[sg.scoresSong.length][];
			for (int i=0; i<sg.scoresSong.length; i++){
				tempSong[i]=new float[sg.scoresSong[i].length];
				System.arraycopy(sg.scoresSong[i], 0, tempSong[i], 0, tempSong[i].length);
			}
		...
		if (matrixcomp){
			...
			if (anasy){
				dsSy=new DisplaySimilarity(sg.scoresSyll, 2, sg, dbc, xd , yd, defaults);
				//int[] popId=sg.getPopulationListArray(2);
				//MRPP mrpp=new MRPP(sg.scoresSyll, popId);
			}
			if (anast){
				dsSt=new DisplaySimilarity(sg.scoreTrans, 3, sg, dbc, xd , yd, defaults);
			}
			if (anaso){
				dsSo=new DisplaySimilarity(sg.scoresSong, 4, sg, dbc, xd , yd, defaults);
		...
		if (treecomp) {
			...
			if (anael){
				if (anacomp){
					Object[] x=makeUPGMA(tempEle, mdsEle.sds, 1, sg.eleLabels);
					dup=(DisplayUPGMA)x[0];
					denpel=(DendrogramPanel)x[1];
				}
				else{
					Object[] x=makeUPGMA(tempEle, mdsEle.sds, 0, sg.eleLabels);
					dup=(DisplayUPGMA)x[0];
					denpel=(DendrogramPanel)x[1];
				}		
			}	
			if (anasy){
				Object[] x=makeUPGMA(sg.scoresSyll, mdsSyll.sds, 2, sg.syllLabels);
				//Object[] x=makeUPGMA(tempSyll, mdsSyll.sds, 2, sg.syllLabels);
				dupsy=(DisplayUPGMA)x[0];
				denpsy=(DendrogramPanel)x[1];
			}
			if (anast){
				Object[] x=makeUPGMA(tempSyTr, mdsSyTr.sds, 3, sg.transLabels);
				dupst=(DisplayUPGMA)x[0];
				denpst=(DendrogramPanel)x[1];
			}
			if (anaso){
				Object[] x=makeUPGMA(sg.scoresSong, mdsSong.sds, 4, sg.songLabels);
				//Object[] x=makeUPGMA(tempSong, mdsSong.sds, 4, sg.songLabels);
		...
		if (hopcomp){
			if (anael){
				CalculateHopkinsStatistic chs=new CalculateHopkinsStatistic(mdsEle.configuration, 1000, 1);
				progress();
				ds.addHopkins(chs);
			}
			if (anasy){
				CalculateHopkinsStatistic chs=new CalculateHopkinsStatistic(mdsSyll.configuration, 1000, 2);
				progress();
				ds.addHopkins(chs);
			}
			if (anast){
				CalculateHopkinsStatistic chs=new CalculateHopkinsStatistic(mdsSyTr.configuration, 1000, 3);
				progress();
				ds.addHopkins(chs);
			}
			if (anaso){
				CalculateHopkinsStatistic chs=new CalculateHopkinsStatistic(mdsSong.configuration, 1000, 4);
				progress();
				ds.addHopkins(chs);
			}
		}
		if(distfunc){
			if (anael){
				if(anacomp){
					DistanceNeighborFunctions dnf=new DistanceNeighborFunctions(sg.scoresEle, 1);
					progress();
					ds.addDistFunc(dnf);
				}
				else{
					DistanceNeighborFunctions dnf=new DistanceNeighborFunctions(sg.scoresEle, 0);
					progress();
					ds.addDistFunc(dnf);
				}
			}
			if (anasy){
				DistanceNeighborFunctions dnf=new DistanceNeighborFunctions(sg.scoresSyll, 2);
				progress();
				ds.addDistFunc(dnf);
			}
			if (anast){
				DistanceNeighborFunctions dnf=new DistanceNeighborFunctions(sg.scoreTrans, 3);
				progress();
				ds.addDistFunc(dnf);
			}
			if (anaso){
				DistanceNeighborFunctions dnf=new DistanceNeighborFunctions(sg.scoresSong, 4);
				progress();
				ds.addDistFunc(dnf);
			}
		...
		if (andcomp){
			...
			if (anael){
				if(anacomp){
					MultivariateDispersionTest mdt=new MultivariateDispersionTest(sg.scoresEle, sg.getPopulationListArray(0), 1, sg.populations, sg.individualEle);
					progress();
					ds.addAnderson(mdt);
				}
				else{
					MultivariateDispersionTest mdt=new MultivariateDispersionTest(sg.scoresEle, sg.getPopulationListArray(0), 0, sg.populations, sg.individualEle);
					progress();
					ds.addAnderson(mdt);
				}
			}
			if (anasy){
				MultivariateDispersionTest mdt=new MultivariateDispersionTest(sg.scoresSyll, sg.getPopulationListArray(2), 2, sg.populations, sg.individualSyl);
				progress();
				ds.addAnderson(mdt);
			}
			if (anast){
				MultivariateDispersionTest mdt=new MultivariateDispersionTest(sg.scoreTrans, sg.getPopulationListArray(3), 3, sg.populations, sg.individualTrans);
				progress();
				ds.addAnderson(mdt);
			}
			if (anaso){
				MultivariateDispersionTest mdt=new MultivariateDispersionTest(sg.scoresSong, sg.getPopulationListArray(4), 4, sg.populations, sg.individualSongs);
				progress();
				ds.addAnderson(mdt);
			}
		}
		...
		if (clustcomp){
			if (anael){
				kmEle=new KMedoids(tempEle, minClusterK, maxClusterK, 1, mdsEle.sds, 10);
				if (popcomp){
					...
					for (int i=minClusterK; i<maxClusterK; i++){
						CompositionAnalyzer compa=new CompositionAnalyzer(kmEle.overallAssignments[i-minClusterK], sg, i, 1, sg.scoresEle);
					}
				}
				progress();
				ds.addCluster(kmEle);
			...
			if (anasy){
				...
				kmSyll=new KMedoids(sg.scoresSyll, minClusterK, maxClusterK, 2, mdsSyll.sds, 10);
				if (popcomp){
					...
					System.out.println(maxClusterK);
					for (int i=minClusterK; i<maxClusterK; i++){
						CompositionAnalyzer compa=new CompositionAnalyzer(kmSyll.overallAssignments[i-minClusterK], sg, i, 2, sg.scoresSyll);
					}
				}
				progress();
			...
			if (anast){
				kmSyTr=new KMedoids(tempSyTr, minClusterK, maxClusterK, 3, mdsSyTr.sds,0);
				if (popcomp){
			...
			if (anaso){
				kmSong=new KMedoids(tempSong, minClusterK, maxClusterK, 4, mdsSong.sds,0);
				if (popcomp){
		...
		if (snncomp){
			if (anael){
				snnEle=new SNNDensity(sg.scoresEle, snnK, snnEps, snnMinPts, 0);
				if (popcomp){
					int[] popIds=sg.getPopulationListArray(0);
			...
			if (anasy){
				//kmSyll=new KMedoids(tempSyll, minClusterK, maxClusterK, 2, mdsSyll.sds);
				snnSyll=new SNNDensity(sg.scoresSyll, snnK, snnEps, snnMinPts, 2);
				if (popcomp){
					int[] popIds=sg.getPopulationListArray(2);
		...
		if (syntcomp){
			sg.calculateIndividuals();
			if (anael){
				entEle=new EntropyAnalysis(sg.scoresEle, maxSyntaxK, sg.individualSongs, sg.lookUpEls, 1, syntaxMode);	
				progress();
				ds.addSyntax(entEle);
			}
			if (anasy){
				...
				entSyll=new EntropyAnalysis(sg.scoresSyll, maxSyntaxK, sg.individualSongs, sg.lookUpSyls, 2, syntaxMode);
	...
	public void displayComparisons(){
		...
		JScrollPane sp=new JScrollPane(sg.ssb, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	public Object[] makeUPGMA(float[][] input, double[] sds, int type, double[] labels){
		...
		for (int i=0; i<dend.dat.length; i++){
			dend.dat[i].colval=dend.dat[i].calculateClusterAverage(labels);
		}
		progress();
