...
public class CompareThreadOld2 extends Thread{
	...
	int[][] proute;
	double[][] dataFocal, dataFocalT, dataFocalE;
	int[][] elPos;
	int[] locsX={-1,-1,0};
	int[] locsY={0,-1,-1};
	
	//ComparisonFrame cf;
	double[][][] data, dataT, dataE;
	...
	public CompareThreadOld2(int maxlength, double[][][]data, double[][][]dataT, double[][][]dataE, int[][]elPos, double[] sds, double sdRatio,  double[] validParameters, boolean weightByAmp, float[] scores, int start, int stop, int f){
	...
	public synchronized void run(){
		...
		for (int i=start; i<stop; i++){
			if (stitch){
				scores[i]=derTimeWarpingAsym(dataFocal, dataFocalT, dataFocalE, data[i], dataT[i], dataE[i], elPos[i]);
			}
			else{
				scores[i]=derTimeWarpingAsym(dataFocal, dataFocalT, dataFocalE, data[i], dataT[i], dataE[i], null);
			}
	...
	public float derTimeWarpingAsym (double[][] dataFocal, double[][] dataFocalT, double[][] dataFocalE, double[][] dataComp, double[][] dataCompT, double[][] dataCompE, int[]pos){
