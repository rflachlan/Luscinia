...
public class SyllableInduction extends JFrame implements ActionListener{
	...
	public SyllableInduction (DataBaseController dbc, Song song, Defaults defaults, MainPanel mp){
		...
		songs[0]=song;
		sg=new SongGroup(songs, defaults);
		
		dtwPanel=new DTWPanel(dbc, sg, false);
	...
	public void compress(){
		...
		double results1[][]=calculateSimilarities(sg.scoresEle, 5);
		...
		for (int i=0; i<phrase_choice.size(); i++){
			...
			if ((x[0]<n)&&(x[1]<n)){
				Element ele1=(Element)song.eleList.get(x[0]);
				Element ele2=(Element)song.eleList.get(x[1]);
			
				int[] p={(int)((ele1.begintime-1)*ele1.timeStep), (int)((ele2.begintime+ele2.signal.length+1)*ele2.timeStep)};
				song.syllList.add(p);
			}
		}
	...
	public void compress2(){	
		...
		for (int i=0; i<size; i++){
			...
			relative_gaps[i]=ele.timeAfter;
			if (i<size-1){av_gap+=ele.timeAfter;}
		}
		av_gap/=size-1.0;
		for (int i=0; i<size; i++){
			Element ele=(Element)song.eleList.get(i);
		...
		for (int i=0; i<size; i++){
			int a=i+phrase_choice[i][0];
			Element ele1=(Element)song.eleList.get(i);
			Element ele2=(Element)song.eleList.get(a);
			...
			if (phrase_choice[i][0]>phrase_choice[i][1]){
				...
				while (!done){
					...
					ele1=(Element)song.eleList.get(c);
					ele2=(Element)song.eleList.get(b);
					//int q[]={ele1.begintime-1, (int)Math.round(ele2.begintime+ele2.length*ele1.timeStep+1)};
					int[] sy={(int)((ele1.begintime-1)*ele1.timeStep), (int)((ele2.begintime+ele2.signal.length+1)*ele2.timeStep)};
					System.out.println("Syllable: "+c+" "+b);
					song.syllList.add(sy);
				}
			}
			i+=phrase_choice[i][0];
