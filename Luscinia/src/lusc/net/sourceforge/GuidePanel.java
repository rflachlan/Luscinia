...
public class GuidePanel extends JPanel implements MouseInputListener{
	...
	public void updateBoundaries(){
		...
		startSeg=(int)Math.round(song.startTime*song.timeStep/timeStep);
		endSeg=(int)Math.round(song.endTime*song.timeStep/timeStep);
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (minx<1000000){
			...
			if (maxx-minx>5){
				minx=(int)Math.round(minx*timeStep/song.timeStep);
				maxx=(int)Math.round(maxx*timeStep/song.timeStep);
				sp.relocate(minx, maxx);
				//System.out.println("MOUSE DRAGGED: "+minx+" "+maxx);
			}
			else{
				int newx=(int)Math.round((minx+maxx)*0.5*timeStep/song.timeStep);
