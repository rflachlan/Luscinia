...
public class DisplayUPGMA extends DisplayPane implements MouseInputListener, PropertyChangeListener, ChangeListener{
	...
	public DisplayUPGMA (UPGMA upgma, SongGroup sg, int dataType, int width, int height, Defaults defaults){
		this.upgma=upgma;
		this.sg=sg;
	...
	public DisplayUPGMA (UPGMA upgma, SongGroup sg, int dataType, int width, int height, double[] silhouettes, double[][] avsils, Defaults defaults){
		this.upgma=upgma;
		this.sg=sg;
		this.dataType=dataType;
		this.width=width;
		...
		for (i=size1; i>=0; i--){
			...
			if (dat[i].children>1){
				for (j=0; j<2; j++){
					...
					else if (silhouettes.length<upgma.dat.length){
						double meancount=0;
						for (k=0; k<dat[q].child.length; k++){
				...
				g.setFont(bodyFont);
				if (dataType==0){
					g.drawString(sg.eleNamesC[dat[i].child[0]], xpl1+5, ypl1+5);
				}
				if (dataType==1){
					g.drawString(sg.eleNames[dat[i].child[0]], xpl1+5, ypl1+5);
				}
				if (dataType==2){
					g.drawString(sg.syllNames[dat[i].child[0]], xpl1+5, ypl1+5);
				}
				if (dataType==3){
					g.drawString(sg.transNames[dat[i].child[0]], xpl1+5, ypl1+5);
				}
				if (dataType==4){
					g.drawString(sg.songNames[dat[i].child[0]], xpl1+5, ypl1+5);
				}
			}
		}
	...
	public void mouseClicked(MouseEvent e) { 
		...
		if (enabled){
			...
			*/
			...
			sg.draw(dataType, dat[loc].child);
	...
	public void stateChanged(ChangeEvent e) {
        ...
        if (RECALC){
        	if (!source.getValueIsAdjusting()) {
        		...
        		cutOffField.setValue(cutoff*upgma.maxDist);
        		paintPanel();
        		spg.paintImage(imf);
        		//repaint();
        		paintDSG();
	...
	public void propertyChange(PropertyChangeEvent e) {
        ...
        if (RECALC){
        	if (source==leavesField){
        		...
        		if (p>upgma.maxDist){
        			p=upgma.maxDist;
        			cutOffField.setValue(upgma.maxDist);
        		}
        		cutoff=(p/upgma.maxDist);
        		setNodePositions();
    			ysize=elements*elespace;
        		RECALC=false;
			...
			if (readyToWrite){
				...
				if (vOut){
					for (int i=size1; i>=0; i--){
						...
						sd.writeDouble(dat[i].dist*upgma.maxDist);
						for (int k=0; k<avsils.length; k++){
				...
				if (cOut){
					...
					for (int i=0; i<upgma.le; i++){
						if (dataType==0){
							sd.writeString(sg.eleNamesC[i]);
						}
						else if (dataType==1){
							sd.writeString(sg.eleNames[i]);
						}
						else if (dataType==2){
							sd.writeString(sg.syllNames[i]);
						}
						else if (dataType==3){
							sd.writeString(sg.transNames[i]);
						}
						else if (dataType==4){
							sd.writeString(sg.songNames[i]);
						}
						...
						//	sd.writeFloat(scores[i][j]);
						//}
						sd.writeLine();
					}
					
					sd.writeSheet("Cluster dist");
					MultivariateDispersionTest mdt=new MultivariateDispersionTest(upgma.input, upgma);
					for (int i=0; i<mdt.clusterDev[1].length; i++){
						if (dataType==0){
							sd.writeString(sg.eleNamesC[i]);
						}
						else if (dataType==1){
						...
						for	(j=1; j<mdt.clusterDev.length; j++){
							sd.writeDouble(mdt.clusterDev[j][i]);
						}
						//for	(j=1; j<scores[i].length; j++){
						//	sd.writeFloat(scores[i][j]);
						//}
				...
				if (rawOut){
					...
					//int[] g=sg.getId(dataType, dat[i].child[j]);
					//String sn1=(String)sg.songs[g[0]].individualName;
					//if (dataType<3){
					//	sd.writeString(sn1+","+sg.songs[g[0]].name+","+(g[1]+1));
					//}
					//else{
					//	sd.writeString(sn1+"'"+sg.songs[g[0]].name);
					//}
					
							if (dataType==0){
								sd.writeString(sg.eleNamesC[dat[i].child[j]]);
							}
							else if (dataType==1){
								sd.writeString(sg.eleNames[dat[i].child[j]]);
							}
							else if (dataType==2){
								sd.writeString(sg.syllNames[dat[i].child[j]]);
							}
							else if (dataType==3){
								sd.writeString(sg.transNames[dat[i].child[j]]);
							}
							else if (dataType==4){
								sd.writeString(sg.songNames[dat[i].child[j]]);
							}
						}
						sd.writeLine();
					}
